...
public class PurchaseOrder extends PageFactory {
    ...
    @AfterMethod
	  ...
	  *  
	  ******************************************************************************************/
	 @Test(retryAnalyzer = RetryAnalyzer.class,dataProvider="DP2",groups={"Purchase","CriticalBatchAndroid"}, description = "Verify when a user is viewing purchase details if there are too many line items to display on the screen at one time the line item total is fixed at the bottom and items scroll under it - Non-Sysco Purchase")
	  ...
	  *  
	  ******************************************************************************************/
	 @Test(retryAnalyzer = RetryAnalyzer.class,dataProvider="DP2",groups={"Purchase","CriticalPatchIOS"}, description = "Given When Then there are too many line items to be viewed on the screen at once a new line item action is selected the new empty line is added to the bottom of the view, extra line items scroll up, under the purchase header so the user can see the new line item - Creating a non-Sysco Purchase")
	 ...
	 /******************************************************************************************
      * Name : INV_PUR_025_VerifyExistenceOfOldSupplier_AfterNewSupplierAddition
      *
      * Description :Verify availability of default supplier name after adding a new supplier.
      *
      * Manual Test cases :INV_PUR_025
      *
      ...
      ******************************************************************************************/
    @Test(retryAnalyzer = RetryAnalyzer.class,dataProvider="DP2",groups={"Purchase"}, description = "As a MA user, I do not want to be able to see non-sysco purchases made. Verify that purchases should not appear under the invoices list.")
    public void INV_PUR_025_VerifyExistenceOfOldSupplier_AfterNewSupplierAddition(PurchaseOrderObject data) throws Exception {
          
            //Login to app
           component().login(data.strUserName, data.strPassword);      
           //Setup inventory with OG , Default location and Default Cat
           component().setupInventorywithOGDefaultLocDefaultCat();
           //Create supplier 1
           component().createSupplier(data.strSupplierName,"","","","","");
           //Click on Purchases
           home().tapPurchases();
           //Verify whether the Purchase page is displayed or not
           purchase().verifyPurchasePage();
           //Validate whether the date is displayed correctly.
           generic().validateDate(0, "MMMM YYYY");
           //Create new purchase and add a new item
           component().createNewPurchaseAndAddLineItem(data.strSupplierName,"INV_PUR_025_VerifyExistenceOfOldSupplier_AfterNewSupplierAddition",data.strInvoiceTotal,data.strLineItemType,data.strLineItemQuantity,1,1);
           //Click on the first item in Purchase page
           purchase().clickOnPurchaseLine("1");
           //Validate whether the purchase information is displayed correctly
           purchase().verifyValidatePurchaseInformation(data.strSupplierName,"","",false);
           //Tap on Done button
           generic().tapDone();
           //Add a new supplier
           component().createSupplier(data.strSupplierNameNew, "", "", "", "", "");
           //Tap on Purchases option from the home page.
           home().tapPurchases();
           //Verify whether all the details in the purchase page is displayed correctly after addition of new supplier
           purchase().verifyPurchasePage();
           //Validate whether the date is displayed correctly after addition of a new supplier.
           generic().validateDate(0, "MMMM YYYY");
           //Click on the first item in the Purchase page after addition of a new supplier.
           purchase().clickOnPurchaseLine("1");
           //Validate that the supplier information in the purchase page is displayed with the same information as set earlier before
            purchase().verifyValidatePurchaseInformation(data.strSupplierName,"","",false);
           //Tap on Done button
            generic().tapDone();
    }
